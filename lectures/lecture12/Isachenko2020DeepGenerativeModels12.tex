...
\begin{frame}{Neural ODE}
	...
	\begin{align*}
	    ...
	    \bz(t_1) &= \int^{t_1}_{t_0} f(\bz(t), \btheta) d t  + \bz_0\quad \Leftarrow \quad \text{solution}.
	\end{align*}
	\vspace{-0.4cm}
...
\end{block}
\vspace{-0.3cm}
If we could get $\frac{\partial L(\by)}{\partial \btheta}$, the problem will be solved.
\vspace{0.2cm}
...
\vspace{-0.3cm}
\end{block}
How to train such model? How to fit $\btheta$? How to compute efficiently $\frac{\partial L}{\partial \btheta}$? -- \textbf{Pontryagin theorem!}
\begin{block}{Adjoint function}
\vspace{-0.2cm}
\[
    \ba(t) = \frac{\partial L(\bz(t))}{\partial \bz(t)}
\]
Shows how the gradient of the loss depends on the hidden state~$\bz(t)$.
\vspace{-0.2cm}
\end{block}
\begin{block}{Theorem}
\vspace{-0.3cm}
...
\begin{frame}{Neural ODE}
	...
	\[
		\ba(t_0) =  - \int_{t_1}^{t_0} \ba(t)^T \frac{\partial f(\bz(t), t, \btheta)}{\partial \bz(t)} dt + \ba(t_1)
	\]
	\end{block}
	\begin{block}{Solution for hidden state}
	\vspace{-0.2cm}
	
	\[
	\bz(t_1) = \int^{t_1}_{t_0} f(\bz(t), \btheta) d t  + \bz_0.
	\]
	\end{block}
	\vfill
	\hrule\medskip
...
%=======
\begin{frame}{Neural ODE}
	\begin{block}{Final gradient}
		\vspace{-0.2cm}
		\[
		\frac{dL}{d \btheta(t_0)} = - \int_{t_1}^{t_0} \ba(t)^T \frac{\partial f(\bz(t), t, \btheta)}{\partial \btheta} dt.
		\]
	\end{block}
	Initial condition is $\frac{\partial L}{\partial \btheta(t_1)} = 0$ (why?). \\
	All these ODEs could be computed simultaneiusly using augmented dynamic.
