...
\begin{frame}{Bayesian framework}
    ...
    \[
        p(\btheta | \bX, \bZ) = \frac{p(\bX, \bZ | \btheta) p(\btheta)}{p(\bX, \bZ)} = \frac{p(\bX, \bZ | \btheta) p(\btheta)}{\int p(\bX , \bZ) p(\btheta) d \btheta} 
    \]
    \end{block}
...
\begin{frame}{Incomplete likelihood}
        \begin{block}{MLE problem}
            ...
            \vspace{-0.3cm}
        \end{block}
	Since $Z$ is unknown, maximize \textbf{incomplete likelihood}.
...
\begin{frame}{Amortized variational inference}
	...
	\begin{itemize}
		\item E-step
		\[
		\bphi_n = \bphi_{n-1} + \left.\eta \nabla_{\bphi} \mathcal{L}(\bphi, \btheta_{n-1})\right|_{\bphi=\bphi_{n-1}}
		\]
		\item M-step
		\[
		\btheta_n = \btheta_{n-1} + \left.\eta \nabla_{\btheta} \mathcal{L}(\bphi_n, \btheta)\right|_{\btheta=\btheta_{n-1}}
		\]
	\end{itemize}
\end{frame}
%=======
