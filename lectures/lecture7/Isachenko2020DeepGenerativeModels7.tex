...
\[
    p(\bx | \bz, \btheta) = \prod_{i=1}^m p(x_i | \bz, \bx_{\text{WindowAround}(i)}).
\]
\begin{itemize}
...
\begin{frame}{Variational Lossy AutoEncoder, 2016}
	...
	\end{block}
	\begin{itemize}
		\item IAF posterior decoder path: $p(\bx|\bz)$, $\bz \sim p(\bz)$.
		\item AF prior decoder path: $p(\bx|\bz)$, $\bz = g(\bepsilon, \blambda)$, $\bepsilon \sim p(\bepsilon)$. 
	\end{itemize}
