	...
	\begin{block}{Optimal generator}
		...
		\begin{multline*}
			V(G, D^*) = \bbE_{\pi(\bx)} \log \frac{\pi(\bx)}{\pi(\bx) + p(\bx | \btheta)} + \bbE_{p(\bx | \btheta)} \log \frac{p(\bx | \btheta)}{\pi(\bx) + p(\bx | \btheta)} \\
			 = KL \left(\pi(x) || \frac{\pi(\bx) + p(\bx | \btheta)}{2}\right) + KL \left(p(x | \btheta) || \frac{\pi(\bx) + p(\bx | \btheta)}{2}\right) - \log 4 \\
			 = 2JSD(\pi(\bx) || p(\bx | \btheta)) - \log 4.
		\end{multline*}
	\end{block}
	\vfill
	\hrule\medskip
...
\begin{frame}{Vanila GAN optimality}
	...
	\[
		V(G^*, D^*) = -\log 4, \quad \pi(\bx) = p(\bx | \btheta).
	\]
...
\end{frame}
%=======
\begin{frame}{Vanila GAN challenges}
	\begin{block}{Objective}
		\vspace{-0.4cm}
		\[
		V(G, D) = \min_{G} \max_D \bbE_{\pi(\bx)} \log D(\bx) + \bbE_{p(\bz)} \log (1 - D(G(\bz)))
	...
	\begin{itemize}
		...
		Maximize $\log D(G(z))$ instead (stronger gradients).
		\item Mode collapse. \\
		The phenomena where the generator of a GAN collapses to one or few samples.
...
\begin{frame}{GAN mode collapse}
	...
	\begin{block}{Likelihood model objective}
		\vspace{-0.6cm}
		\begin{multline*}
			\max_{\btheta} \log p(\bX | \btheta) \approx \max_{\btheta}\bbE_{\pi(\bx)} \log p(\bx | \btheta) = \\ = \max_{\btheta}\bbE_{\pi(\bx)} \log p(\bx | \btheta) + \bbE_{\pi(\bx)} \log \pi(\bx) = \\ = \min_{\btheta} KL(\pi(\bx) || p(\bx | \btheta))
		\end{multline*}
	\vspace{-0.6cm}
	\end{block}
	\begin{figure}
		\centering
		\includegraphics[width=0.8\linewidth]{figs/mode_collapse_1}
	\end{figure}
\vfill
\hrule\medskip
{\scriptsize \href{https://arxiv.org/abs/1511.01844}{https://arxiv.org/abs/1511.01844}}
...
\begin{frame}{Mode collapse}
	\begin{block}{Mode covering vs mode seeking}
		\begin{figure}
			\centering
			\includegraphics[width=0.8\linewidth]{figs/mode_collapse_2}
		\end{figure}
	\vspace{-0.3cm}
	\end{block}
	\begin{block}{GAN mode collapse on toy example}
		\begin{figure}
			\centering
	...
	\vfill
	\hrule\medskip
	{\scriptsize \href{https://sites.google.com/view/berkeley-cs294-158-sp20/home}{https://sites.google.com/view/berkeley-cs294-158-sp20/home} \\ \href{https://arxiv.org/abs/1611.02163}{https://arxiv.org/abs/1611.02163}}
\end{frame}
%=======
